...
// Create ticket with XSS sanitization
router.post('/', requireAuth, async (req, res) => {
  ...
  try {
    // FIX 4.9: Sanitize HTML to prevent XSS
    ...
    const result = await pool.query(
      ...
      [title, sanitizedDescription, severity, userId]
    );
...
// FIX 4.7: IDOR - Verify ownership before returning ticket
router.get('/:id', requireAuth, async (req, res) => {
  ...
  try {
    ...
    // Check ownership: MANAGER can view any, ANALYST only their own
...
// FIX 4.8: SQL Injection - Use parameterized queries
router.get('/search/query', requireAuth, async (req, res) => {
  ...
  try {
    ...
    const result = await pool.query(
      'SELECT * FROM tickets WHERE owner_id = $1 AND (title LIKE $2 OR description LIKE $3)',
...
router.put('/:id', requireAuth, async (req, res) => {
  ...
  try {
    ...
    if (description !== undefined) {
      // FIX 4.9: Sanitize description to prevent XSS
